...
pub fn calculate_risk_score(tx_count: u32, total_amount: u64, days_active: u64) -> u8 {
    ...
    let freq_score = if frequency > 500 {
        40
    } else {
        (frequency * 40 / 500) as u8
    };
    let amount_score = if daily_avg > 1_000_000 {
        40
    } else {
        (daily_avg * 40 / 1_000_000) as u8
    };
    let consistency = if tx_count > 100 && days_active > 7 {
        20
    } else {
        10
    };
...
pub fn validate_slot_range(start: u64, end: u64) -> Result<u64> {
    ...
    require!(
        range <= 432_000 * 30,
        crate::errors::LuumError::WindowOverflow
    );
    Ok(range)
}
